<?php 
	...
	foreach ($total as $row) {
		...
		$affich.= '</td>';
		foreach ($larequete as $niv) {
			$exist = $connexion->query("SELECT * FROM Act_Attrib WHERE (No_Niv_Attrib='".$niv['No_Niv']."' AND No_Act_Attrib='".$row['No_Act']."');");
	...
	$affich.= "document.querySelector('form').onsubmit = function(e){";
		...
		$affich.= "}";
  	$affich.= 'SendJson(selected, "http://duss.alwaysdata.net/suivi/qcm/back/api/attribAN.php");';
	$affich.= "}";
	$affich.= "</script>";
